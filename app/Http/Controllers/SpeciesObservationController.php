...
class SpeciesObservationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($allResults->isEmpty()) {
            Log::warning('No observation fact rows for current filters, returning empty results');
        ...
        try {
            ...
            if (! empty($validated['site_name_id'])) {
                ...
                if ($protectedArea->code === 'PPLS') {
                    ...
                    $siteTableName = $this->observationUnion->createSiteTableName($siteName->name, $siteName->id);
    ...
    private function handleExport(Request $request)
    {
        ...
        $query = null;
        $batanesQuery = $this->observationUnion->buildFilteredQuery(BmsSpeciesObservation::class, $request);
        $fuyotQuery = $this->observationUnion->buildFilteredQuery(FuyotObservation::class, $request);
        $quirinoQuery = $this->observationUnion->buildFilteredQuery(QuirinoObservation::class, $request);
        $palauiQuery = $this->observationUnion->buildFilteredQuery(PalauiObservation::class, $request);
        $buaaQuery = $this->observationUnion->buildFilteredQuery(BauaObservation::class, $request);
        $wangagQuery = $this->observationUnion->buildFilteredQuery(WangagObservation::class, $request);

        // For MPL, include all sub-tables
        $magapitQuery = $this->observationUnion->buildFilteredQuery(MagapitObservation::class, $request);
        $marianoQuery = $this->observationUnion->buildFilteredQuery(MarianoObservation::class, $request);
        $madupapaQuery = $this->observationUnion->buildFilteredQuery(MadupapaObservation::class, $request);
        ...
        if ($request->filled('site_name') && $request->site_name !== 'no_specific_site') {
            ...
            if ($siteName) {
                ...
                if ($stationCode) {
                    if (strpos($siteName->name, 'San Mariano') !== false) {
                        $query = $this->observationUnion->buildFilteredQuery(MarianoObservation::class, $request);
                    } elseif (strpos($siteName->name, 'Madupapa') !== false) {
                        $query = $this->observationUnion->buildFilteredQuery(MadupapaObservation::class, $request);
                    } else {
                        $query = null;
                    }
                }
        ...
        $madreQuery = $this->observationUnion->buildFilteredQuery(MadreObservation::class, $request);
        $tumauiniQuery = $this->observationUnion->buildFilteredQuery(TumauiniObservation::class, $request);
        $banganQuery = $this->observationUnion->buildFilteredQuery(BanganObservation::class, $request);
        $salinasQuery = $this->observationUnion->buildFilteredQuery(SalinasObservation::class, $request);
        $dupaxQuery = $this->observationUnion->buildFilteredQuery(DupaxObservation::class, $request);
        $casecnanQuery = $this->observationUnion->buildFilteredQuery(CasecnanObservation::class, $request);
        $dipaniongQuery = $this->observationUnion->buildFilteredQuery(DipaniongObservation::class, $request);

        // Add PPLS site-specific tables
        $toyotaQuery = $this->observationUnion->buildTableQuery('toyota_tbl', $request);
        $sanRoqueQuery = $this->observationUnion->buildTableQuery('roque_tbl', $request);
        $mangaQuery = $this->observationUnion->buildTableQuery('manga_tbl', $request);
        $quibalQuery = $this->observationUnion->buildTableQuery('quibal_tbl', $request);
