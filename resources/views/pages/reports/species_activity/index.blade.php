...
<div class="species-obs-summary-cards mb-6">
    <div class="kpi-grid">
        ...
        <div class="kpi-card kpi-card--purple"><div class="kpi-card-icon kpi-card-icon--purple"><i data-lucide="map-pin" class="lucide-icon"></i></div><div class="kpi-card-body">@if (! empty($isPaScoped))<p class="kpi-card-label">Total Sites</p><p class="kpi-card-value">{{ number_format($summaryStats['total_sites'] ?? 0) }}</p><span class="kpi-card-meta kpi-card-meta--neutral">sites monitored</span>@else<p class="kpi-card-label">Total Protected Areas</p><p class="kpi-card-value">{{ number_format($summaryStats['total_protected_areas'] ?? 0) }}</p><span class="kpi-card-meta kpi-card-meta--neutral">unique areas</span>@endif</div></div>
...
<div class="action-bar-card">
    ...
    <div class="data-table-wrap"><div class="responsive-table-container data-table-container"><table class="responsive-table species-activity-table"><thead><tr><th class="species-activity-col-rank">Rank</th><th>Species Name</th><th>Scientific Name</th>@unless($isPaScoped)<th class="species-activity-col-num">Protected Areas</th>@endunless<th class="species-activity-col-num">Recorded Count</th><th class="species-activity-col-num">Observation Records</th></tr></thead><tbody>
    @forelse($rows as $index => $row)
        <tr class="data-table-row {{ $index % 2 === 0 ? 'data-table-row--even' : 'data-table-row--odd' }}"><td class="species-activity-col-rank"><span class="species-activity-rank-badge">{{ $row->rank }}</span></td><td><span class="data-table-cell-truncate font-medium">{{ $row->species_name ?: '—' }}</span></td><td><em class="data-table-cell-truncate">{{ $row->scientific_name ?: '—' }}</em></td>@unless($isPaScoped)<td class="species-activity-col-num">{{ number_format($row->protected_area_count) }}</td>@endunless<td class="species-activity-col-num">{{ number_format($row->recorded_count_sum) }}</td><td class="species-activity-col-num">{{ number_format($row->observation_frequency) }}</td></tr>
    @empty
        <tr><td colspan="{{ $isPaScoped ? 5 : 6 }}" class="data-table-empty-cell"><div class="data-table-empty-state"><i data-lucide="clipboard-list" class="lucide-icon data-table-empty-icon"></i><h3 class="data-table-empty-title">No activity records found</h3><p class="data-table-empty-text">Adjust filters or add observations from the Species Observations page.</p></div></td></tr>
    @endforelse
    </tbody></table></div></div>
    @if ($rows->total() > 0)
